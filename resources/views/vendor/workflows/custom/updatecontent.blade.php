      ...
      <div class="tab-content" id="pills-tabContent">
        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
          ...
          <form action="{{route('updatecontent.save')}}" class="form-horizontal" enctype="multipart/form-data" method="post">
            ...
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Key Name</label>
            ...
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">
                {{ strtoupper($item->name) }}</label>
              <input type="text" class="form-control" name="{{ $item->name }}" minlength="7" maxlength="15" size="15" pattern="^((\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.){3}(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$">
            ...
            <div class="mb-3">
              ...
              @endphp
              <select name="{{ $item->name }}" class="form-control" type="checkbox" id="">
              ...
              @foreach ($valuelist as $item1)
              <input name="{{ $item->name }}" value="{{ $item1 }}" id="nonr" class="form-check-input" type="radio">
              <label for="{{ $item1 }}">
              ...
              @endphp
              @foreach ($valuelist as $item1)
              <input type="checkbox" class="form-check-input" name="{{ $item->name }}[{{ $item1 }}]">
              <label for="{{ $item1 }}">
                {{ strtoupper($item1) }}</label><br>
              @endforeach
              @endif
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <div class="mb-3 text-start">
                          ...
                          <select name="{{ $item->name }}[user_list][]" id="" class="form-control" multiple>
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <div class="mb-3 text-start">
                          <label for="message-text" class="col-form-label fw-bold text-left ">Groups
                            <small>(ctrl + click) multiple select</small> </label>
                          <select name="{{ $item->name }}[group_list][]" id="" class="form-control" multiple>
